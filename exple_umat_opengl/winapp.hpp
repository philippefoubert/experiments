...
public:
    ...
    float time(UNITS i_unit = MSEC)
    {
        float sec = m_diff / m_tick_frequency;
        return sec * m_unit_mul[i_unit];
    }
...
{
public:
    WinApp(int i_width, int i_height, const std::string &i_window_name)
    {
        m_width       = i_width;
        m_height      = i_height;
...
#if defined(WIN32) || defined(_WIN32)
    virtual LRESULT CALLBACK WndProc(HWND i_hWnd, UINT i_message, WPARAM i_wParam, LPARAM i_lParam) = 0;
#endif
...
#if defined(WIN32) || defined(_WIN32)
    static LRESULT CALLBACK StaticWndProc(HWND i_hWnd, UINT i_message, WPARAM i_wParam, LPARAM i_lParam)
    {
        WinApp* pWnd;

        if (i_message == WM_NCCREATE)
        {
            LPCREATESTRUCT pCreateStruct = ((LPCREATESTRUCT)i_lParam);
            pWnd = (WinApp*)(pCreateStruct->lpCreateParams);
            ::SetWindowLongPtr(i_hWnd, GWLP_USERDATA, (LONG_PTR)pWnd);
        }

        pWnd = GetObjectFromWindow(i_hWnd);

        if (pWnd)
            return pWnd->WndProc(i_hWnd, i_message, i_wParam, i_lParam);
        else
